...
const MonthlyDocumentsManager: React.FC<{
    ...
    const handleFileDrop = async (file: File, type: 'poz' | 'invoice') => {
        if (!file) return;
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                <div>
                    ...
                    {pozFileContent ? (
                        <div className="flex items-center gap-2 p-4 border rounded-md bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300">
                            <p className="text-sm truncate flex-1">Załadowano: {pozFile?.name || 'POZ.pdf'}</p>
                        </div>
                    ) : (
                        <Dropzone
                            onFileDrop={(file) => handleFileDrop(file, 'poz')}
                ...
                <div>
                    ...
                    {invoiceFileContent ? (
                        <div className="flex items-center gap-2 p-4 border rounded-md bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300">
                            <p className="text-sm truncate flex-1">Załadowano: {invoiceFile?.name || 'Faktura.pdf'}</p>
                        </div>
                    ) : (
                        <Dropzone
                            onFileDrop={(file) => handleFileDrop(file, 'invoice')}
                            accept="application/pdf"
                            maxSizeMB={1.5}
                            disabled={isMerging}
                        />
