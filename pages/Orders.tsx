...
const Orders: React.FC<OrdersProps> = ({ orders, clients, addOrder, updateOrder, deleteOrder }) => {
  ...
  useEffect(() => {
    if (orderData.clientId) {
      const isOrderingPersonValid = availableContacts.some(contact => contact.name === orderData.orderingPerson);
      if (!isOrderingPersonValid && orderData.orderingPerson !== '') {
        setOrderData(prev => ({...prev, orderingPerson: ''}));
      }
    }
  }, [orderData.clientId, availableContacts, orderData.orderingPerson]);
  ...
  const handleAttachmentDrop = async (file: File) => {
    if (!file) return;
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Select 
                    label="Osoba Zamawiająca" 
                    name="orderingPerson" 
                    value={orderData.orderingPerson} 
                    onChange={handleInputChange}
                    disabled={!orderData.clientId || availableContacts.length === 0}
                  >
