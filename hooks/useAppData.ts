...
import type { Client, Order, Settlement, SettlementItem, MonthlyDocument, Contact } from '../types';
import { OrderStatus, OrderItemType } from '../types';

const MOCK_CLIENTS: Client[] = [
  { id: 'c1', name: 'Global Tech Inc.', nip: '123-456-78-90', phone: '111-222-333', contacts: [
      { id: 'ct1', name: 'Alice Johnson', email: 'alice.j@globaltech.com'},
      { id: 'ct2', name: 'Bob Williams', email: 'bob.w@globaltech.com'},
  ] },
  { id: 'c2', name: 'Innovate Solutions', nip: '098-765-43-21', phone: '444-555-666', contacts: [
      { id: 'ct3', name: 'Charlie Brown', email: 'charlie.b@innovate.com' },
  ] },
];
...
export function useAppData() {
  ...
  const addClient = (client: Omit<Client, 'id'>) => {
    const newClient = { 
        ...
        contacts: client.contacts.map(c => ({ ...c, id: crypto.randomUUID() }))
    };
    setData(prev => ({ ...prev, clients: [...prev.clients, newClient] }));
  };
  
  const updateClient = (updatedClient: Client) => {
    const clientWithContactIds = {
        ...updatedClient,
        contacts: updatedClient.contacts.map(c => ({ ...c, id: c.id || crypto.randomUUID() }))
    };
    setData(prev => ({ 
        ...prev, 
        clients: prev.clients.map(c => c.id === clientWithContactIds.id ? clientWithContactIds : c) 
    }));
  };
